...
`cargo build --release` builds the `ssbh_data_py` module that can be imported into Python. The resulting binary can only be used with the current Python version, processor type, and operating system. Depending on the platform, it may be necessary to rename the file. See the [PyO3 page](https://github.com/PyO3/pyo3) for details. In most cases, the directory containing the built module will need to be added to the Python path, which can be done with `sys.path.append(path_to_module_directory)`. 
